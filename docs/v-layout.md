...
截至 2026-09-26，“布局概览”只投影背景层（1）和顶层（4）中可按原始参数绘制的自定义图片。参考视口为 1920×1080；平台默认 Windows，可切换 iOS／Android。可拖动图片来修改原组件的 `dx`／`dy` 偏移量，点击“保存皮肤”才写回包。拖拽保留原有 X／Y 锚点、单位、尺寸及其他字段。Lua 仍可能在运行时改写这些模块的位置或尺寸；它用于检查原始参数和资源，不是游戏运行截图。
