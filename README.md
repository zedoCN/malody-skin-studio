...
马老弟音游的非官方皮肤编辑与预览工具。支持打开 4.x `.mui`／`.msz` 和 Malody V `.msp`／`info.asm`。`.mui` 使用 JavaFX 画布预览组件与动画、切换设备比例和时间轴；V 皮肤可编辑元数据和部分组件参数，预览选中资源及同层静态图片布局。V 编辑器的“Lua 源码”页可编辑 `info.asm` 引用的 UTF-8 脚本，点击“保存皮肤”一同写回；缺失或无效文件会显示诊断，不会执行脚本。
