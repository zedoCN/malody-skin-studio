...
public class ZXLogger {
    ...
    private static final java.util.logging.Logger logger = java.util.logging.Logger.getLogger(ZXLogger.class.getName());
    ...
    private static void log(Level level, String message, int cutoff) {
        ...
        StackTraceElement ste = getStackTraceElement(ZXLogger.class.getName(), cutoff);
        assert ste != null;
        record.setSourceMethodName(ste.getMethodName());
        record.setParameters(new Object[]{ste.getLineNumber()});
        record.setSourceClassName(ste.getClassName());
