...
function Init()
    ...
    if string.find(uimod.Text,"Death") or getjudge>10 then
        non3bool=true
    end
    Newflickbool=Module:GetBool("新版单划键判定")
    maxstvalue=10000
    ...
    if non3bool then
        STbool="Nope"
        STState=3
    elseif (Module:GetBool("自动ST")==true) then
        STbool="AutoST"
        STstate=0
    else
    ...
    elseif rawJson then
        ...
        for i=1,mcnotecount do
            ...
            elseif mcnote.type==7 then
                if (mcnote.segments[0].time==0 and mcnote.segments.Length>1) then
...
function OnDrawNote(note)
    ...
    if (note.type<=7 and note.y<200) then
        note.y=200
        if #notelist[notetruex]>0 then
            local temp_x=#notelist[notetruex]
            while note.time<notelist[notetruex][temp_x].time do
                temp_x=temp_x-1
                if temp_x==0 then
                    break
                end
            end
            table.insert(notelist[notetruex],temp_x+1,note)
        else
	...
	--flick
        ...
        if note.extras then
            flickhead=mod:AddSprite("flick","headpoint.png")
        else
    ...
    elseif (note.type==7) then
        ...
        if note.extras then
            head=mod:AddSprite("head","notefbody0.png","group")
        else
...
function OnNoteMove(note,mod,p)
    ...
    elseif (note.type==7) then
        ...
        if (holdbreakid[note.extra]~=nil or holdnoteid[note.extra]~=note.extra) then
			...
			--if LN break, replace textures
			    if note.extras then
			        head=mod:AddSprite("head","notefbody0break.png")
			    else
                    head=mod:AddSprite("head",KNum.."notelbreak"..SegX(note)..".png")
                end
            end
            ...
            mod.Y=932*(1-xv[1])
            if note.extras then
                head.Width=240*notescale[1]
                head.Height=240*notescale[1]
            else
                head.Width=NoteW*notescale[1]
                head.Height=116*notescale[1]
            end
	...
	-- tap or move (key down)
		...
		-- set press state if current finger is not occupied
				...
				-- if is a flick
                    if (flickstate[source+1]~=0) then
                        if (fingerid[source+1]==SegX(flicknote[source+1])+flicknote[source+1].arrow-10) then
                            ...
                            if Newflickbool then
                                flickoffset=flicktime[source+1]-flicknote[source+1].time
                            else
                                flickoffset=time-flicknote[source+1].time
                            end
                            FlickJudge()
                            JudgeModule()
                            Combo()
                    ...
                    if (holdmap[source+1]>0 and holdstate[holdmap[source+1]]~=0 and holdstate[holdmap[source+1]]~=9999 and (not (holdstate[holdmap[source+1]]==holdnote[holdmap[source+1]].segments.Length and holdnote[holdmap[source+1]].segments.Length>1 and SegX(holdnote[holdmap[source+1]],-2)==SegX(holdnote[holdmap[source+1]],-1)))) then
                        ...
                        --if finger out of the hitbox
							if (holdnote[holdmap[source+1]].segments.Length==1 and holdnote[holdmap[source+1]].x==holdnote[holdmap[source+1]].segments[0].x) then
							--single hold LN
			...
			--LN head judgement, always set sperfect
                ...
                holdcombovalue[holdmap[source+1]]=1				
                if (notelist[notex][notecur[notex]-1].segments[0].time>0 or notelist[notex][notecur[notex]-1].segments.Length==1) then
...
function Update()
    ...
    for i=1,segsource+1 do
        if (flickstate[i]==1) then
            if (uptime-flicknote[i].time>200*judgescale) then
...
function Score()
    ...
    uiscore.Text=math.floor(scorevalue*ScoreRate)
...
function FinishScore()
    if (datascorevalue~=scorevalue) then
        ...
        Play:SetScore(math.floor(datascorevalue*ScoreRate))
    end
end
function FinishAcc()
...
function AutoNote()
		    ...
		    if (itemhold>0) then
				...
				for i=1,20 do
					...
					if (holdstate[i]==0 or holdstate[i]==9999) then
						...
						lnnotecombo[i]=0
						if (noteinfo.segments[0].time>0 or noteinfo.segments.Length==1) then
							lnnotecombo[i]=1
							ljudgevalue=math.floor(judgevalue)
							if (STstate==0) then
							    judgevalue=2.3
	...
	--auto mode
        if not (noteinfo.type==7 and noteinfo.segments[0].time==0 and noteinfo.segments.Length>1) then
            ...
            elseif jsontable[i][k].attr==3 then
                ...
                if jsontable[i][k].isend==1 then
                    ...
                    while snoteend==false do
                        snoteend=true
                        if snotetable[1].track==snotetable[1].totrack then
                            for n=1,KNum do
                                for m=1,#jsontable[n] do
                                    if jsontable[n][m].tick==snotetable[1].tick and jsontable[n][m].isend==0 and jsontable[n][m].track~=jsontable[n][m].totrack and jsontable[n][m].totrack==snotetable[1].track then
                                        table.insert(snotetable,1,jsontable[n][m])
                                        snoteend=false
                                        goto jsonskip1
                                    end
                                end
                            ...
                            ::jsonskip1::
                            if snotetable[1].attr==3 then
                                snoteend=true
                            end
                        else
                            local nextpart=snotetable[1].track-2
                            for m=1,#jsontable[nextpart] do
                                if jsontable[nextpart][m].tick+jsontable[nextpart][m].dur==snotetable[1].tick and jsontable[nextpart][m].isend==0 and jsontable[nextpart][m].track==jsontable[nextpart][m].totrack then
                                    table.insert(snotetable,1,jsontable[nextpart][m])
                                    snoteend=false
                                    goto jsonskip2
                                end
                            end
                    ...
                    local jsonnote={
                        ...
                        x=snotetable[1].track-2,
                        y=10,
                        segments={}
                    }
                    if snotetable[1].attr~=3 then
                        jsonnote.extras="nohead"
                    end
                    notecombolist[jsonnote.extra]=0
                    if snotetable[1].totrack~=snotetable[1].track then
                        if #snotetable>1 then
                            ...
                            local jsonsnote={
                                ...
                                x=jsonnote.x,
                                y=101,
                                arrow=snotetable[1].totrack-snotetable[1].track+10
                            }
                            Note:AddVirtual(jsonsnote)
                        else
                            jsonnote.type=5
                            jsonnote.arrow=snotetable[1].totrack-snotetable[1].track+10
                        end
